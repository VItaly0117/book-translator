...
\[
(\Gamma Y) \quad \begin{cases}$\frac{\partial u\left(0,\,t\right)}{\partial x} = \frac{h}{k}\left$[u\left(0,\,t\right) - g_{1}$$\left(t\right)\[
\right], \\
\]\frac{\partial u\left(L,\,t\right)}{\partial x} = -\frac{h}{k}\left$[u\left(L,\,t\right) -$g_{2}$$\left(t\right)\right]. \end{cases} \quad 0 < t < \infty,
\]
...
\[
\begin{aligned} & (\mathsf{У}\mathsf{Ч}\Pi) & \[
u_t = \alpha^2 u_{xx}, \\
\]& (\mathsf{\Gamma}\mathsf{Y}) & \begin{cases} \alpha_1 $u_x$ \, (0, \, t) + \beta_1 u \, (0, \, t) = $g_1$ \, (t) \\ \alpha_2 $u_x$ \, (L, \, t) + \beta_2 u \, (L, \, t) = $g_2$ \, (t) \end{cases} \text{ (неоднородные } \mathsf{\Gamma}\mathsf{Y}), \\ & (\mathsf{H}\mathsf{Y}) & u \, (x, \, 0) = \varphi \, (x) \end{aligned}
\]
...
\[
a_$n = \frac{2}{L} \int_{-\infty}^{L} \overline{\phi}(\xi) \sin(\pi n\xi/L) d\xi$.
\]
...
\[
a_$n = \frac{2\lambda_n}{(\lambda_n - \sin \lambda_n \cos \lambda_n)} \int_0^{\infty} \xi \sin (\lambda_n \xi) d\xi$.
\]
...
методом преобразования к более простому уравнению. На що похоже розв'язок? Можна дать следующую интерпретацию цієї задачи: $u(x, t)$— концентрация некоторого вещества в движущейся среде (среда движется слева направо со скоростью v=1), начальное распределение копцентрации задастся функцией $e^{x/2}$ , на границах концентрация поддерживается нулевой (можна считать, що там стоят фильтры). Соответствует ли ваще розв'язок такой интерпретации?
...
а також визначити реакції системи $u_n$\left(x,\,t\right) на вплив кожного компонента f_$n\left(t\right)$X_$n\left(x\right)$ . Якщо знайти всі відповіді, рішення проблеми виглядатиме так:
...
\[
\begin{aligned} (n=1) & $T_1'$ + (\pi\alpha)^2 \[
T_1 = 0 \\ T_1(0) = 1
\]& \Rightarrow $T_1$(t) = $e^{-(\pi\alpha)^2 t}$, \\ (n=2) & $T_2'$ + (2\pi\alpha)^2 $T_2 = 0 \T_2\ (0) = 0$& \Rightarrow $T_2$(t) = 0, \\ (n=3) & $T_3'$ + (3\pi\alpha)^2 \[
T_3 = 4 \\ T_3(0) = 0
\]& \Rightarrow $T_3$(t)$= \frac{1}{(3\pi\alpha)^2} \left$[1 - $e^{-(3\ pi\alpha)^2 t}$\right], \\ (n \geqslant 4) & $T_n'$ + (n\pi\alpha)^2 \[
T_n = 0 \\ T_n(0) = 0
\]& \Rightarrow $T_n$(0) = 0. \end{aligned}
\]
...
\[
\begin{split} &\mathcal{F}_s\left[$u_{xx}$\right]$= \frac{2}{\pi} \omega u \ (0, \t) - \omega^2 \mathcal{F}_s\left$[u\right] = \\ &\[
= \frac{2}{\pi} \omega u \ (0, \t) - \omega^2 U \ (t) = \\
\]&= \frac{2A\omega}{\pi} - \omega^2 U \ (t). \end{split}
\]
...
\[
\begin{aligned} (O \Pi V) &\[
\frac{dU}{di} + \omega^2 \alpha^2 U = \frac{2A\omega \alpha^2}{\pi}, \\ (H V)
\]& $U(0)$ = 0. \end{aligned}
\]
...
ЦЕЛЬ ЛЕКЦИИ: Ввести понятие ряда Фурье и показать, каким образом периодическую функцию $f(x)$ можна представить в гиде ряда по синусам и косинусам:

\[
f(x)$$= \frac{$a_0$}{2} + \sum_{n=1}^{\infty}$[$a_n$\cos(n\pi x) +$b_n \sin(n\pi x)].
\]

Якщо же функція $f(x)$ непериодическая и определена на (-\infty, +\infty) , то показать, как ряд Фурье переходит в *преобразование Фурье* и как функцию $f(x)$ можна разложить в непрерывную совокупность простейших функцій. Эго разложение (интеграл Фурье) можна записать в комплексной форме:
...
\[
a_$n = \frac{1}{L} \int_{-L}^{L}$f(x)$\cos(n\pi x/L) dx = 0, \quad n = 0, 1, 2, \dots$,
\]

\[
b_$n = \frac{1}{L} \int_{-L}^{L}$f(x)$\sin(n\pi x/L) dx = -(-1)^n (2L/n\pi), \quad n = 1, 2, \dots$.
\]

Эти ингегралы легко вычисляются. Чтобы вывести формулы Эйлера для коефіцієнтів $a_n$ и $b_n$ , умножим обе части разложения (11.1) на \sin{(n\pi x/L)} или \cos{(n\pi x/L)} и проинтегрируем получив- Р И С
...
Тот факт, що сумма ряда Фурье определена и периодична вне интервала [0, 1], ничему не противоречит, поскольку мы интересуемся поведением функції лише внутри интервала. На самом деле для функції, определенной внутри интервала, можна указать много различных разложений в ряд Фурье, поскольку функцию можна продолжить за пределы интервала различными способами. Сразу же отметим, що полученные ряды будут сходиться к исходной функції с различными скоростями.

Читатель не повинен думать, що всякую периодическую функцию можна разложить в ряд Фурье. Пока мы знаем лише, що якщо функцию $f(x)$ можна разложить в ряд Фурье (11.1), то коефіцієнти $a_n$ и $b_n$ находятся по формулам Эйлера (11.2). Более того, даже якщо функція разложима в ряд Фурье, мы не знаем, можна ли ряд почленно дифференцировать. И в самом деле, легко видеть, що производную функції $f(x)$ = x (пилообразной функції) нельзя найти почленным дифференцированием ряда Фурье (11.3).

В литературе, перечисленной в конце книги, можна найти точные условия разложимости функції $f(x)$ в ряд Фурье, а також условия почленной дифференцируемости получающегося ряда. Нам буде достаточно ознакомиться с известной теоремой Дирихле.

*Теорема Дирихле* (достаточные условия разложимости функції в ряд Фурье):

Якщо $f(x)$—ограниченная периодическая функція, имеющая на каждом периоде конечное число максимумов, минимумов и точек разрыва, то ряд Фурье функції $f(x)$ сходится к $f(x)$ в каждой точке непрерывности функції и к среднему значению пределов функції слева и справа в каждой точке разрыва функції $f(x)$.
...
Теперь мы педголовлены для знакомства с преобразованием Фурье. Однако целесообразно буде познакомиться сначала с понятием частотного спектра периодической функції.

# Дискретный частотный спектр периодической функції.

Якщо $f(x)$ — периодическая функція, то разложение в ряд Фурье можна интерпретировать как представление функції $f(x)$ последовательностью чисел \{$c_n$\} вида

\[
$c_n = \sqrt{a_n^2 + b_n^2}, \quad n = 0, 1, 2, \dots$,
\]

где коефіцієнти $c_n$ служат мерой вклада соответствующих частотных компонент в функцию $f(x)$. Наприклад, пилообразная функція $f(x)$ разлагается в ряд Фурье

\[
f(x)$$= \frac{2L}{\pi} \left$[ \sin(\pi x/L) -$\frac{1}{2} \sin(2\pi x/L) + \frac{1}{3} \sin(3\pi x/L) - \dots \right]
\]

и, отже, частотный спектр \{$c_n$\} записывается в виде $c_n=2L/n\pi$для n=1,\ 2,\ \dots и $c_0$=|$a_0$|=0 (см. рис. 11.2).

![](images/_page_84_Figure_8.jpeg)

PNC. 11 2. Дискретный частотный спектр пилообразной функції a— основная частота (период 2L); b—n-ая гармоника (период 2L/n).
...
лише по частотам, кратным основной частоте, как це було у периодических функцій). По аналогии с рядами Фурье определим частотный спектр по формуле
...
яке принято называть интегральным представлением Фурье. Отсюда получаем пару інтегральних перетворень
...
В следующей лекции мы остановимся на изучении свойств цих преобразований.
...
где $U(t)$ = \mathcal{F}[$u(x, t)$] . Читате ть повинен обратить внимание на то, що функція $U(t)$ на салот доле зависит не лише от t, но и от \xi , но, поскольку в дифференциальном уравнении (12.7) величина \xi играет роль константы, мы не будем записывать ее в число аргументов, т. е. будем считать, що U = $U(t)$.

ШАГ 2 (Розв'язок преобразованной задачи).

Поскольку новая переменная играет роль гонстанты в задаче (12.7), ее розв'язок легко находится:
...
ШАГ 3 (Нахождение обратного преобразования). Искомое розв'язок $u(x, t)$ находится по формуле
...
Дело сделано! Формула (12.9) дает розв'язок нашей задачи. Перш ніж чем поставить точку, давайте проанализируем полученный результат. Отметим, що почынтегральная функція состои с

из двух сомножителей:

1. Начальной температури $q(x)$,

    2. Функції G(x, t) = \frac{1}{2\alpha \sqrt{\pi t}} e^{-(x-\xi)^2/4c^2t} , яка називається функцией Грина или функцией источника.

Можна показать, що функція источника $G(x, t)$ описывает отклик системи на начальный температурный единичный импульс Р И С
...
Главный недостаток преобразования Фурье в том, що его можна применять не ко всем функциям. Наприклад, функції вида
...
расходится. Фурье-образ имеют лише те функції, які достаточно быстро сгремятся к нулю при |x| \to \infty . Кроме того, нельзя применять преобразование Фурье по времени для розв'язки задач с начальными условиями, так как 0 < t < \infty (якщо не рассматривать продолжения функцій на полуось в соответствующем классе функцій).

# ЗАДАЧИ

1. Найдите фурье-образ функції
...
а формулы для преобразования похідних $u_t$ и $u_{tt}$ можна получить интегрированием по частям.

# Свойство 3 (Теорема о свертке)

Свертка играет здесь ту же роль, що и в теории преобразования Фурье, но вводится несколько иначе.
...
\[
\begin{aligned} (\text{YHI}) & \[
u_t = -2u_x, \\ (\text{HY})
\]& $u_t$(x, 0) = $e^{-x^2}$. \end{aligned}
\]
...
\[
\begin{split} u\left(x,\,t\right) &= \$phi\left(x-ct\right)$ + \\[
psi\left(x+ct\right) = \\
\]&$= \frac{1}{2} \left$[ $f\left(x-ct\right)$ + $f\left(x+ct\right)$ \right] +\frac{1}{2c} \int\limits_{x-ct}^{x+ct} g\left(\xi\right) \, d\xi. \end{split}
\]
...
\[
\begin{aligned} (\mathrm{Y} \mathrm{H} \Pi) & \[
u_{tt} = \alpha^2 u_{xx} + Kx, \quad 0 < x < 1, \ 0 < t < \infty, \\ (\mathrm{\Gamma} \ma
\]thrm{Y}) & \begin{cases} u \ (0, \ t) = 0, \\ u \ (1, \ t) = 0, \end{cases} & 0 < t < \infty, \\ (\mathrm{H} \mathrm{Y}) & \begin{cases} u \ (x, \ 0) = f \ (x), \\ $u_t$ \ (x, \ 0) = 0. \end{cases} & 0 \leqslant x \leqslant 1, \end{aligned}
\]
...
MATHBLK0793X  $u_{xx}$(1, t) = 0.  Используя теорию тонких балок, можно показать, что изгибающий момент в балке пропорционален величине   $u_{xx}$  , а изгибающий момент в свободно опирающемся конце должен быть ра- Р И С
...
\[
\begin{aligned} & (\text{Y} \cdot \Pi) \quad \[
u_{tt} = -u_{xxxx}, \quad 0 < x < 1, \quad 0 < t < \infty, \\
\]& \begin{cases} u \cdot (0, \ t) = 0, \\ $u_{xx}$ \cdot (0, \ t) = 0, \\ u \cdot (1, \ t) = 0, \\ $u_{xx}$ \cdot (1, \ t) = 0, \end{cases} \quad 0 < t < \infty, \\ & \begin{cases} u \cdot (x, \ 0) = $f(x)$, \\ $u_{1}$(x, \ 0) = $g(x)$. \end{cases} \quad 0 \leqslant x \leqslant 1, \end{aligned}
\]
...
\[
\begin{split} u\left(0,\ t\right) &= 0 \Rightarrow X\left(0\right)$$T\left(t\right) = 0 \Rightarrow X\left(0\right) = 0 \Rightarrow C + E = 0 \\ $u_{xx}$\left(0,\ t\right)$&= 0 \Rightarrow X''$\left(0\right)$$T\left(t\right) = 0 \Rightarrow X$''$\left(0\right) = 0 \Rightarrow -C + E = 0 \\ u\left(1,\ t\right)\[
&= 0 \Rightarrow D\sin\sqrt{\omega} + F\sin\sqrt{\omega} = 0, \\
\]u_{xx}\left(1,\ t\right) &= 0 \Rightarrow -D\sin\sqrt{\omega} + F\sin\sqrt{\omega} = 0. \end{split}
\]
...
\[
a_$n = \frac{2}{(n\pi)^2} \int_0^1$g(x)$\sin(n\pi x) dx$,
\]
...
\[
\begin{aligned} \[
u_x = u_\xi \xi_x + u_\eta \eta_x, \\ u_y = u_\xi \xi_y + u_\eta \eta_y, \\ (23.4) \ u
\]_{xx} = $u_{\xi\xi}$ \xi_x^2 + 2u_{\xi\eta} \xi_x \eta_x + $u_{\eta\eta}$ \eta_x^2 + $u_{\xi}$ \xi_{xx} + $u_{\eta}$ \eta_{xx}, \\ $u_{xy} = u_{\xi\xi} \xi_x^2 + u_{\xi\eta} (\xi_x \eta_y + \xi_y \eta_x) + u_{\eta\eta} \e$ta_x \eta_y + $u_{\xi}$ \xi_{xy} + $u_{\eta}$ \eta_{xy}, \\ $u_{yy} = u_{\xi\xi} \xi_y^2 + 2u_{\xi\eta} \xi_y \eta_y + u_{\eta\eta} \eta_y^2 + u_{\xi}$ \xi_{yy} + $u_{\eta}$ \eta_{yy}. \end{aligned}
\]
...
\[
T = \int_{0}^{T} dt = \int_{a}^{L}`\frac{dt}{ds} ds = \int_{0}^{L} \frac{ds}{v} = \frac{1}{\sqrt{2mg}} \int_{0}^{L} \frac{ds}{\s`qrt{y}} = \frac{1}{\sqrt{2mg}} \int_{a}^{b} \sqrt{\frac{1+$y'^{2}$}{y}} dy
\]
...
\[
\begin{array}{l} u_{\xi} = u_{\alpha}\alpha_{\xi} + u_{\beta}\beta_{\xi} = u_{\alpha} + u_{\beta}. \\ u_{\eta} = $u_{\alpha}$\alpha_{\eta} + $u_{\beta}$\beta_{\eta} = $u_{\alpha}$ - $u_{\beta}$, \\ $u_{\xi\eta} = u_{\alpha\alpha}\alpha_{\eta} + u_{\alpha\beta}\beta_{\eta} + u_{\beta\alpha}\al$pha_{\eta} + $u_{\beta\beta}$\beta_{\eta} = $u_{\alpha\alpha}$ - $u_{\beta\beta}$, \end{array}
\]
...
\[
\begin{aligned} (\text{YAII}) & \[
u_{tt} = c^2 \Delta u, \\ (\text{HY})
\]& \begin{cases} u = 0, \\ u_t = \varphi, \end{cases} \end{aligned}
\]
...
\[
\Delta u = $u_{rr}$ +$\frac{2}{r}$u_r$+ \frac{1}{r^2}$u_{\theta\theta}$+ \frac{\cot\theta}{r^2}$u_0`+ \frac{1}{r^2\s`in\theta}$u_{\phi\phi}$
\]
...
\[
\begin{aligned} & (\forall \mathsf{Y}\Pi) \quad \[
u_{tt} = c^2 u_{xx}, \\
\]& (\mathsf{H} \mathsf{Y}) \quad \begin{cases} u\left(x, \ 0\right) = 0, \\ $u_t$\left(x, \ 0\right) = \$phi\left(x\right)$. \end{cases} \end{aligned}
\]
...
2. Результаты задачи 1 используйте для розв'язки следующей задачи:
...
3. Проиллюстрируйте и прокомментируйте розв'язок трехмерной задачи
...
Отметим, що в обратном косинус-преобразовании суммирование начинается с n=0, а в синус-преобразовании — с n=1.
...
(Заметим, що преобразование проводилось по переменной x, и полученная последовательность зависит лише от времени t.)
...
рівняння теплопровідності с поглощением рівняння теплопровідності с источником
...
\[
\begin{split} \frac{d^2S_$n\left(t\right)$}{dt^2} &= -\left(\pi n\right)^2S_$n\left(t\right)$ + 2n\pi\left[u\left(0,\ t\right) + $\left(-1\right)$^{n+1}u\left(1,\ t\right)\right] + D_\[
n\left(t\right) = \\
\]&= -\left(n\pi\right)^2S_$n\left(t\right)$ + D_$n\left(t\right)$, \end{split}
\]
...
Якщо теперь преобразовать початкові умови краевой задачи, то получим початкові умови для обыкновенного дифференциального уравнения
...
1. Для розв'язки задачи методом конечного синус- или косинуспреобразования краевые условия при x=0 и x=L повинні иметь вид
...
Перед нами неоднородное рівняння теплопровідності, так що метод поділу змінних неприменим. Можпо, конечно, воспользоваться конечным синус-преобразованием по переменной x или преобразованием Лапласа по переменной t, но вместо цього мы рассмотрим две подзадачи
...
Каждая из цих задач легко решается и в то же время ясно, що  \mathit{suma}  решений задач  $P_1$  и  $P_2$  является решением исходной вадачи, т. е.

\[
\varphi(r) = a \ln r + b, \quad r = u^2 + v^2.
\]

Розв'язок задачи  $P_{i}$  Розв'язок задачи  $P_i$  Розв'язок задачи  $P_i$  Можна показать, що в общем случае розв'язок задачи
...
\[
\begin{cases} $u_t = u_{xx} +$f(x, t), \\u(0, t)\[
= 0, \\
\]u(1, t)\[
= 0, \\
...
Отметим, во-первых, що коефіцієнти  $A_n$  ,  $B_n$  ,  $F_n$  действительно являются функциями времени t, поскольку мы применяли синус-преобразование к функції двух змінних x, t, u, вовторых, що мы разложили входное воздействие $f(x,t)$ на простые компоненты  $F_n$(t)  . Найдем теперь отклики  $U_n$(t)  на каждую момпоненту  $F_n$(t)  , а затем сложим все отклики  $U_n$(t)  и получим розв'язок $u(x,t)$.

Чтобы найти отклики  $U_n$(t)  , необходимо проделать небольшие выкладки с коэффициентами  $A_n$(t)  и  $B_n$(t)  , чтобы в подынтегральные вирази входила лише функція u, а не ее производные  $u_i$  и  $u_{xx}$  . Интегрируя по частям, получаем

Функція Хевісайду,

\[
\begin{split} $A_n$(t) &= 2\int_0^1 $u_t$ \sin(n\pi x) \, $dx = \frac{d}{dt} \bigg$[ 2\int_0^1 $u(x, t)$ \sin(n\pi x) \, dx \bigg]\[
= \frac{dU_n(t)}{dt} \,, \\
\]B_n$(t)$&= 2\int_0^1 $u_{xx}$ \sin(n\pi x) \, dx = -(n\pi)^2 \, $U_n$(t) \,+ \\ &\quad + 2n\pi \, \big[ $u(0, t)$ + (-1)^{n+1} \, $u(1, t)$ \big], \end{split}
\]
...
Таким образом, мы получили связь между входом и выходом системи, между коэффициентами  $F_n$  и  $U_n$  . Перш ніж чем решать уравнения для  $U_n$(t)  , обратимся к начальному условию
...
,  n = 1,  2, \dots  Мы получили разложение нашей смешанной задачи на ряд простых задач об отклике системи на элементарные входные воздействия:
...
(HУ)  $U_n$(0) = 0, n = 1, 2, ...  Каждое из цих уравнений легко решается методом интегрирующего множителя (или с помощью преобразования Лапласа). В любом случае получаем
...
Читатель, должно быть, заметил, що каждый отклик  $U_n$(t)  умножается на весовой множитель  \sin(n\pi x)  . Конечно, в разложении функції $f(x, t)$ на компоненты  $F_n$(t)  присутствуют те же весовые множители.

# ЗАМЕЧАНИЕ

Применение конечного синус-преобразования позволяет записывать розв'язок в виде ряда, по для большинства других інтегральних перетворень розв'язки представляются в виде интегралов (непрерывное разложение). Читателю можна предложить дать интерпретацию задачи
...
3. Предположим, що  $u_1$  и  $u_2$  — розв'язки следующих уравнений:
...
\mathbf{\Pi}  ля каких уравнений функція  $u_1$ + $u_2$  також буде решением? Какие выводы вы сможете сделать, исходя из ваших ответов?

4. Найдите четыре смещанные задачи, сумма решений которых дает розв'язок следующей задачи:
...
6. Предположим, що функції  $u_1$  и  $u_2$  удовлетворяют линейным однородным граничным условиям

\[
$\frac{\pi}{2}$e^{-a+\omega}$$
\]

$u_x$(1, t) + $h_2$ $u(1, t)$ = 0.  Буде ли функція  $u_i$ + $u_i$  удовлетворять этим условиям? , .

Лекція 27

# УРАВНЕНИЯ ПЕРВОГО ПОРЯДКА (МЕТОД ХАРАКТЕРИСТИК)

ЦЕЛЬ ЛЕКЦИИ: Ввести понятие уравнения з частинними похідними первого порядка (до сих пор мы рассматривали уравнения второго порядка) и познакомиться с важным методом розв'язки задач с начальными условиями—методом характеристик. Задача, которой мы теперь займемся, имеет вид
...
,  -\infty < x < \infty  ,  0 < t < \infty  константа  \alpha^2  играла роль коэффициента диффузии, а v—была скоростью среды. Очевидно, що якщо  \alpha=0  (т. е. диффузии нет), то розв'язок буде смещаться как целое вдоль оси x со скоростью v (у нас осталась лише конвекция). Другими словами, якщо початкова умова  $u(x,0)$=\varphi(x)  , то соответствующее розв'язок уравнения

\[
$\frac{\pi}{2}$e^{-a+\omega}$$
\]

буде иметь вид  $u(x, t)$ = \varphi(x-vt)  .
...
Конечно, якщо a и b—константы, то розв'язок представляет собой бегущую с постоянной скоростью волну. Якщо же $a(x, t)$ и $b(x, t)$
...
зависят от x и t, то скорость потока изменяется как вдоль потока, так и во времени (читатель сможет увидеть, що начальная кривая сильно искажается). Здесь так много аналогий с конвекцией!
...
(НУ)  $u(x, 0)$ = \varphi(x), -\infty < x < \infty.  Розв'язок цього линейного уравнения первого порядка основывается на следующем физическом факте: початкова умова в некоторой точке x переносится в tx-плоскости вдоль линии, яка

називається характеристи-кой (см. рис. 27.1)

Этим наше уразнение отличается от других уравнений (таких, как рівняння теплопровідності  $u_t = u_{xx} )$,для которых начальное значение в точке x оказывает влияние на розв'язок во всех точках пространства и во все моменты времени. Якщо читатель помнит, то начальное сме- Р И С
...
Рассмотрим сначала новую координату s. Якщо s выбрать так, що она удовлетворяет приведенному выше условию, то уравнение
...
Другими словами, на характеристиках  \{[$x(s)$, $t(s)$]: 0 < s < \infty\}  уравнение з частинними похідними становится обыкновенным дифференциальным уравнением. Поясним эти идеи на конкретном примере.

# ПРИМЕР

Предположим, що нам потрібно решить следующую задачу Коши с постоянными коэффициентами:
...
\[
S_$n = \frac{2}{\pi} \int_0^{\pi}$f(x)$\sin(nx) dx$.
\]
...
\[
\begin{aligned} (\mathsf{Y}\mathsf{P}\Pi) & $u_t$ + g\left(u\right)$$u_x = 0\[
,& -\infty < x < \infty, & 0 < t < \infty, \\ (\mathsf{H}\mathsf{Y}) & u\left(x, \ 0\right) = \
\]varphi\left(x\right), & -\infty < x < \infty. \end{aligned}
\]
...
\[
\begin{cases}`\frac{\partial`u_1`}{\partial x} =`u_2, \\ \frac{\partiala_1`}{\partial t} =`u_3, \\ \frac{\partialu_3$}{\partia$l t} = c^2`\frac{\partial`u_2}{\partial x} + au_3 + bu_4. \end{cases}
\]
...
\[
\begin{bmatrix}`\frac{\partial`u_1\[
...
\[
\begin{cases} $a_0$ + $b_0$ \ln R_1$$= \frac{1}{2\pi} \int\limits_0^{2\pi} $g_1$(s) \, ds \\ $a_0$ + $b_0$ \ln R_2 = \frac{1}{2\pi} \int\li$$mits_0^{2\pi} $g_2$(s) \, ds \end{cases}
\]
...
\[
\begin{split} $u_x$(x,\ y) & \\[
cong \frac{u(x+h,\ y)-u(x,\ y)}{h}, \\
\]u_{xx}$(x,\ y)$& \$cong \frac{1}{h^2} \big$[ u(x+h,\ y)-2u(x,\ y)+$u(x-h,y)$ \big], \\ $u_u$(x,\ y) & \\[
cong \frac{u(x,\ y+k)-u(x,\ y)}{k}, \\
\]u_{uy}$(x,\ y)$& \$cong \frac{1}{h^2} \big$[ u(x,\ y+k)-2u(x,\ y)+u(x,\ y-k) \big]. \end{split}
\]
...
также можно построить явную разностную схему Для этого аппроксимируем  $u_{tt}$  и  $u_{xy}$  центральными разностными произ- Р И С
...
Из (38.5) видно, что для вычисления решения на данном времениом слое необходимо знать решение на двух предыдущих слоях. Значит, для начала счета необходимо воспользоваться Р И С
...
Після того как ця частная задача буде решена методом Монте-Карло, мы обсудим, как решать этим методом более общие задачи.

Для иллюстрации метода Монте-Карло рассмотрим игру, яка називається «Блуждающий пьяница». Чтобы играть в эту игру, необходим стол с нанесенной на нем сеткой из линий, как показано на рис. 43.1.

Перейдем к описанию правил игры.

# Как играть в «Блуждающего пьяницу»?

ШАГ 1. Блуждания пьяницы начинаются из произвольной точки сетки (в нашем случае це точка A).

![](images/_page_321_Figure_4.jpeg)

- А начальная точка - - внутренние точки сетки - □ -Конечная точка р;

- $g_i$ вознаграждение в m . $p_I$ Рис. 43.1. Игровое поле для игры «Блуждающий пьяница».

ШАГ 2. На каждом ходе игры пьяница случайным образом перебирается в одну из четырех соседних точек сетки (в нашем случае соседними для A являются точки B, C, D и E и вероятность попадания в каждую из цих точек равна 0,25).
...
ШАГ 3. Після перехода в соседнюю точку процесс блуждания возобновляется. Так пьяница идет от точки к точке до тех пор, пока случайным образом не окажется в граничной точке $p_t$ . Здесь он останавливается, а мы записываем номер точки $p_t$ . На цьому заканчивается одна случайная прогулка.
...
Оказывается, що среднее вознаграждение, о котором лише що шла речь, является приближенным решением задачи Дирихле в точке A. Це интересное наблюдение основано на двух фактах.

1. Предположим, що пъяница начал свою прогулку из точки, лежащей на еранице. Каждая такая прогулка немедленно заканчивается в той же точке, и пъяница получает вознаграждение $g_i$ . Таким образом, среднее вознаграждение для каждой граничной точки равно $g_i$ .

2. Теперь предположим, що прогулка нач нается из внутренней точки. Тогда ясно, що среднее вознаграждечие для точки $R(A)$ буде средним арифметическим от средних вознаграждений для четырех соседних точек

\[
R(A)$$= \frac{1}{4}$[$R(B)$+$R(C)$+$R(D)$+$R(E)].
\]

Зададимся еще раз вопросом: почему же среднее значение вознаграждения $R(A)$ аппроксимирует розв'язок задачи Дирихле в точке A? Мы видели, що величина $R(A)$ удовлетворяет двум
...
Якщо $g_i$ —це значения функції $g(x, y)$ из граничного условия в граничных точках $p_i$ , то два наших уравнения точно совпадают с двумя уравнениями, які були получены при решении задачи Дирихле методом конечных разностей. То есть величина $R(A)$ соответствует величине $u_{i,f}$ в разностных уравнениях

\[
u_{i,j}$$= \frac{1}{4} \left$[$u_{i-1,j}$+$u_{i+1,j}$+$u_{i,j-1}$+$u_{i,j+1} \right],
\]

(i, j)—внутренняя точка, $u_{ij} = g_{ij}, g_{ij}$—значение розв'язки в граничной точке (i, j).

Отже, величина $R(A)$ действительно аппроксимирует розв'язок уравнения з частинними похідними в точке A.

Теперь можна считать, що игра в «Блуждающего пьяницу» состоит из трех шагов.

# Розв'язок уравнения Лапласа методом Монте-Карло

- Приведенные ниже правила позволяют получить розв'язок в одной внутренней точке квадрата.
...
- ШАГ 2. Після того как все прогулки завершены, для каждой граничной точки вычисляем относительное число прогулок, завершившихся в цієї точке. Обозначим эти величины $P_A$($p_i$) .

    - ШАГ 3. Вычисляем приближенное розв'язок u(A) по формуле u(A) = g_1 P_A(p_1) + g_2 P_A(p_2) + \dots + g_N P_A(p_N) ,

где $g_i$ — значение функції g в точке $p_i$ , а N — число граничных точек.

Игру «Блуждающий пьяница» можна модернизировать, чтобы решить более сложную задачу. Ниже приводится пример такой модернизации.
...
и подставим их в уравнение з частинними похідними. Соответствующая сетка показана на рис. 43.2. Разрешив получившееся уравнение относительно $u_{i,l}$ , получаем

\[
u_{i, j} = \frac{u_{i, j+1} + u_{i, j-1} + \sin x_j (u_{i+1, j} + u_{i-1, j})}{2 (1 + \sin$$x_j)}.
\]

Посмотрим внимательно на це уравнение. Коефіцієнти при $u_{i+1,\ j}$,\ $u_i$,\ j+i,\ $u_l$,\ j-1 н $u_{i-1}$,\ j положительны, и их сумма равна единице. Другими словами, розв'язок $u_i$,\ j является взвешенным средним решений в четырех соседних точках. Отже, можна Р И С

![](images/_page_324_Picture_9.jpeg)

. 43.2. Узлы сетки для случайного маршруга.

модифицировать «Блуждающего пьяницу» так, чтобы вероятности перехода в соседние точки були не 0,25, а равнялись бы коэффициенту в соответствующем члене. Другими словами, якщо пьяница находится в точке (i, j), то он переходит в точку
...
В остальном игра не меняется. Модификация для других задач может оказаться более хитроумной, но основная идея остается той же. Читатель может заняться разработкой своих игр для розв'язки других задач. Параболический случай рассмотрен в разделе «Задачи».
...
2. Во многих случаях исследователя интересует розв'язок уравнения з частинними похідними в одной точке. Якщо граница области сложна и уравнение зависит от трех или четырех независимых змінних, метод Монте-Карло может оказаться удобным. Методы Монте-Карло возникли при решении очень трудных задач диффузии нейтронов. Эти задачи невозможно решать аналитически.

# ЗАДАЧИ

1. Построить блок-схему игры «Блуждающий пьяница» для розв'язки задачи
...
- во внутренних узлах сетки, якщо число узлов сетки произвольно. 2. Напишите программу для розв'язки задачи по блок-схеме из задачи 1.
...
\[
\begin{aligned} $u_{xx}$ + \[
u_{yy} = 0, \quad 0 < x < 1, \quad 0 < y < 1, \\
\]u(x, 1)$= 0, \quad 0 < x < 1, \quad 0 <$y < 1, \\ $u(x, 0)$ = 0, \quad 0 < x < 1, \quad 0 < y < 1, \\ $u(0, y)$ = 1, \quad \frac{\partial u}{\partial x}(1, y) = 0 \end{aligned}
\]
...
Отметим, що в первых двух задачах коефіцієнти постоянны. Читатель повинен понимать, що параметр в повинен быть достаточно мал, ниаче бескопечный ряд может оказаться расходящимся
...
Ця форма понадобится нам позже. В цьому примере она интересна тем, що показывает, как гиперболы в плоскости z переходят в координатные линии u = const и v = const в плоскости w.

Теперь мы перейдем к изучению одного частного вида функцій комплексного переменного, які осуществляют конформные отображения.
...
Приведем несколько примеров, в которых буде показано, как можна решать уравнения з частинними похідними с помощью конформных отображений.

Рівняння лапласа в верхней полуплоскости.

Предположим, що нам необходимо решить следующую задачу Дирихле в верхней полувлоскости (рис. 47.3):
...
\[
\begin{aligned} \[
u_{tt} = c^2 u_{xx} \\ u_{tt} = c^2 u_{xx} - h u_t \\ u_{tt} = c^2 u_{xx} - h u_t - k u \
\]\ $u_{tt} = c^2 u_{xx} +$f(x, t)\end{aligned}
\]
...
2. Термофізичні характеристики матерії. 3. Радянський математик, академік, автор найвідомішого підручника з рівнянь математичної фізики. 4. Задача з граничними умовами третього типу називається задачею ___________
    ...
    Функція піни H(t) = \begin{cases} 0, & t < 0 \\ 1, & t \ge 0 \end{cases} \end{cases}
