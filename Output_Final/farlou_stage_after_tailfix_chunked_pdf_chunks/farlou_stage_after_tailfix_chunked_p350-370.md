...
Задача була зведена до знаходження двох функцій $ \xi(x, y) $ і $ \eta(x, y) $ , щоб відношення $ [\xi_x/\xi_y] $ і $ [\eta_x/\eta_y] $ задовольняли рівняння (23.7). Дуже легко знайти такі функції 1) якщо звернути увагу на рис. 23.1. Щоб зрозуміти, як це можна зробити, розглянемо просте рівняння
...
- 4. Решите задачу 1 с новым граничным условием при x=1 $ u_x(1, t) = -[u(1, t) 1]. $ # НЕЯВНЫЕ РАЗНОСТНЫЕ СХЕМЫ (СХЕМА КРАНКА—НИКОЛЬСОНА)
...
де вводиться позначення $ r = k/h^2 $ . Зверніть увагу, що якщо i фіксоване, а j змінюється від 2 до n-1, співвідношення (39.4) визначають систему n-2 рівнянь з n-2 невідомим $ u_{t+1,2}, u_{t+1,3}, $
...
Перед нами розв'язок задачи конвективной диффузии (15.2). Его очень легко интерпретировать, якщо представить, що мы движемся относительно графика, изображенного на рис. 15.6.
...
де константы $ \alpha $ и $ \beta $ определяются из условий y(a) = A и y(b) = B (т. е. цепная линия должна проходить через заданные точки). Решить уравнение Эйлера—Лагранжа в данном случае достаточно трудно, поэтому мы рекомендуем читателю лише проверить, що цепная линия ему удовлетворяет.
...
ри цьому оказывается, що $ J[\overline{y}] = 0.46 $ . Якщо подставить в функционал любую другую гладкую функцию, график которой проходит через точки (0, 0) и (1, 1), то значение функционала J[y] станет больше.
...
В це время можна численно найти эначение функционала $ J[u_n] $ . Якщо необходимо найти значение розв'язки $ u_n(x,y) $ в некоторой точке, можна написать небольшую программу эля быполнения отой операции. При необходимости можна повторить обчислення по цієї программе с другими значениями п

РИС 452. Блок-схема метода Ритца.

# РОЗВ'ЯЗОК УРАВНЕНИЙ З ЧАСТИННИМИ ПОХІДНИМИ МЕТОДАМИ ТЕОРИИ ВОЗМУЩЕНИЙ

ЦЕЛЬ ЛЕКЦИИ: Показать, как различные сложные задачи (нелинейные уравнения с змінними коэффициентами, области неправильной формы и т. д.) можна решить методом возмущения более простых задач. Иными словами, мы хотим показать, как следует изменить розв'язок простой задачи, чтобы оно давало приближенное розв'язок трудной задачи.
...
Як вправу, читача просять знайти пертурбацію першого порядку $ u_1 $ і перевірити, наскільки добре є приблизне розв'язання я

![](images/_page_189_Picture_10.jpeg)

кщо розглянути рівняння з параметром
...
а фундаментальные розв'язки и, (удовлетворяют уравнению и граничным условиям (21.2)) записываются в виде
...
А теперь, поскольку уравнение и граничні умови линейны и однородны, можна утверждать, що сумма
...
також удовлетворяет уравнению и граничным условиям. Единственное, що нам осталось—це выбрать коефіцієнти $ a_n $ и $ b_n $ так, чтобы удовлетворить начальным условиям. Подставляя (21.4) в початкові умови, получаем
...
Таким образом, розв'язок задачи имеет вид (21.4), а коефіцієнти $ a_n $ и $ b_n $ вычисляются по формулам (21.6)

Чтобы помочь читателю разобраться в цієї задаче, приведем простой пример.

# Модель колеблющейся балки

Рассмотрим свободно опирающуюся балку, изображенную на рис. 21.2. Пусть початкові умови таковы:

$$
y = \frac{\pi}{b-a}(x-a).
$$ $ u_1(x, 0) = 0 $ Мы могли бы найти розв'язок, подставив функції f(x) и g(x) в формулы (21.6), однако легко видеть, що
...
Интересно сравнить це розв'язок с решением для струны при тех же начальных условиях. Якщо заглянуть в лекцию 20, там можна найти розв'язок соответствующей задачи для струны:
...
Другими словами, балка колеблется на более высоких частотах, чем струна. Отметим, що в том и другом случаях высшие частоты кратны основным частотам колебаний.

# ЗАМЕЧАНИЯ

- 1. Вообще говоря, балку можна закрепить тремя способами:
...
$$ $ u_x(0, t) = 0, $ $ u_{xx}(1, t) = 0, $ $ u_{xx}(1, t) = 0, $ розв'язок уже не буде выражаться в виде ряда из произведений синусов и косинусов. Оно буде иметь более сложную структуру

$$ где власні функції $ X_n(x) $ являются линейными комбинациями синусов, косинусов, гиперболических синусов и гиперболических косинусов. Розв'язок цієї задачи можна найта в руководствах по теории упругости.
