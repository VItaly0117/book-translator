...
Припустимо, наприклад, що залежність потоку від густини є квадратичною, тобто $f(u) = u^{2}$ , тоді закон збереження набуває наступної форми:
...
тоді рівняння характеристик набуває форми $ x = x_0 + g[u(x_0,  0)] t $ (рівняння характеристики, що починається з точки $ (x_0, 0) $ ).
...
Розглянемо модель руху, у якій потік обчислюється за формулою $ f(u)=u^2 $ , і початковою густиною iso-P і C
...
Ці прямі лінії показані на рис. 28.4. Тепер ми бачимо точки $ 0 < x_0 < 1 $ . Тут характеристики позначаються такими словами:
...
= $ x_0 + g[0]t = $ = $ x_0 $ підвести до вертикальних ліній, scho vyhodat z x<sub>o</sub>. Характеристики завдань показані на рис. 28.4.

![](images/_page_208_Figure_10.jpeg)

Фіг. 28.4. Характеристики рівняння $ u_t + 2uu_x = 0 $ .

Ситуація на поверхні на 0 < t < 1/2 для завершення ситуації. Рис. 28.5 показує розвиток нашої проблеми в інший момент і годину. Щоб повірити, що характеристики сходяться в одній точці t=1/2. З того ж для розвитку задачі на t>1/2 необхідно розробити інший метод. Якщо характеристики сходяться в певній точці, йдеться про вплив хвилю (розвитку розвитку).
...
# ПОВАГА
1. Ударна хвиля в наному приладі винікає чер тхо, що потік швидко зростоє зі збілшенням густини у. Якчо потік задажник з формулою f(u) = u, тоді ривняня було у вигляді $ u_1+u_x=0 $ , і розв'язок ц'его ривняннє представляє хвилю, шо, рахое в правтовані без спотов'. Очевидно, що значення формули f(u) = u. слід пояснити таким обрядом.
...
Тепер перейдемо до основної частини нашої лекції: визначення функції Гріна та розв'язку рівняння (36.1).

Однак спершу слід ввести поняття потенціалу гострих джерел і стічних вод.

# Потенціали точкових джерел і стічних вод
...
створено одним точковим джерелом (пошук тривимірного потенціалу залишаємо читачеві як вправу).

Припустимо, що єдине точкове джерело +q розташоване в початку Clear that heat (або щось інше)

буде виходити з області вздовж радіальних ліній, і тому для повного потоку, що проходить через коло радіуса r, можна записати
...
Але загальний потік має співпадати з кількістю тепла, що утворюється всередині кола (закон збереження), тобто
