...
i(x, t) — розподіл струму вздовж проводу, $ v\left(x,\ t\right) $ — розподіл потенціалу вздовж проводу,
...
Ймовірність завершення випадкового блукання в граничній точці $p_i$ і величина винагороди $g_i$ | Гранична точка p <sub>i</sub>                        | $P_{\mathcal{A}}\left(p_{i}\right)$ — відносна частка блукань, що завершилися в точці $p_{i}$ | $g_i$ — величина винагороди при досягненні точки $p_i$ |  |  |
|-------------------------------------------------------|---------------------------------------------------------------------------------------------------|------------------------------------------------------------|--|--|
...
Гра закінчується, коли знайдено значення $R\left(A\right)$ . Відповідно до даних таблиці. 43.1 за $R\left(A\right)$ отримуємо
...
$$ $u_{xx} + (\sin x) u_{yy} = 0$ , $0 < x < \pi$ , $0 < y < \pi$ , $(\Gamma Y)$ $u(x, y) = g(x, y)$ на межі квадрата.
...
і $_{yy}=u_{l+1,\ j}-2u_{l,\ j}+u_{l-1,\ j},$ (центральні різницеві похідні),
...
Давайте розглянемо це рівняння уважніше. Коефіцієнти при $u_{i+1,\ j},\ u_i,\ j+i,\ u_l,\ j-1$ і $u_{i-1},\ j$ додатні, і їхня сума дорівнює одиниці. Іншими словами, $u_i,\ j$ є зваженим середнім розв'язків у чотирьох сусідніх точках. Отже, можливо
...
Інакше гра не змінюється. Модифікація для інших завдань може бути більш винахідливою, але основна ідея залишається незмінною. Читач може почати розробляти власні ігри, щоб розв'язувати інші проблеми. Параболічний випадок розглядається в розділі «Проблеми».
# ПОВАГА
1. Зверніть увагу, що якщо відносні частоти удару $P_A(p_l)$ відомі, то значення $u\left(A\right)$ за інших граничних умов $g_l$ легко знайти. Для цього достатньо підставити $g_l$ у формулу
...
2. У багатьох випадках дослідник зацікавлений у розв'язанні диференціального рівняння в певній точці. Якщо межа області є комплексною і рівняння залежить від трьох або чотирьох пояснювальних змінних, метод Монте-Карло може бути зручним. Методи Монте-Карло виникли для розв'язання дуже складних задач дифузії нейтронів. Ці завдання не можна розв'язати аналітично.
# ЗАВДАННЯ
1. Побудуйте блок-схему гри «Блукаючий п'яниця», щоб розв'язати проблему
...
КРОК 1 (Замінити координати (x, t) на нові канонічні координати $ (\xi, \eta) $ ).
...
\begin{aligned}
(\text{РЧП})\quad & u_t = \alpha^2 u_{xx} - \beta u_x - \gamma u, && 0 < x < 1,\ 0 < t < \infty, \\
...
Підставляючи ці вирази для похідних у хвильове рівняння, отримаємо
...
— довільна функція від $ \eta $ .

Інтегрування останнього співвідношення за **η** приводить до загального розв'язку
...
$$ $ u(\xi, \eta) = 1/\eta + tg \xi, $ $ u(\xi, \eta) = \eta^2 + e $ є розв'язками рівняння $ u_{\mathrm{ }nh}=0 $ . Другий крок зроблено. $ \text{IIIA}\Gamma $ 3 (Повернення до старих координат x і t).

Для знаходження загального розв'язку, тобто всіх розв'язків рівняння $ u_{tt} = c^2 u_{xx} $ , підставимо
...
Це загальний розв'язок хвильового рівняння. З фізичної точки зору це цікаво, оскільки воно представляє суму двох хвиль P і C
...
приводять, загалом кажучи, до нерізкого заднього фронту (оскільки формула Д'Аламбера передбачає інтегрування функції $ \psi(x) $ у межах від x-ct до x+ct).

Тепер покажемо, що принцип Гюйгенса не застосовується до циліндричних хвиль. Такі рішення описують хвилі на воді з точкового джерела, коли задній край хвилі не є гострим, а поступово зникає до нуля.

# Двовимірне хвильове рівняння

Щоб розв'язати двовимірну хвильову задачу
...
- 3. Аналітичний розв'язок дозволяє визначити розв'язок у будь-якій точці, а не лише на вузлах сітки.
...
u = функція від параметрів.

Тоді можна поставити питання про визначення параметрів як функцій вхідних даних за схемою

Параметри = функція від u = функція вихідних даних.

Таку задачу називають параметричною ідентифікацією. Для параметричної ідентифікації потрібно вміти розв'язувати рівняння з частковими похідними. Трохи далі в цій лекції ми наведемо приклад параметричної ідентифікації в біології, але спочатку подивимося, чим корисні чисельні розв'язки.

# Переваги чисельних розв'язків

Головна перевага чисельних розв'язків полягає в тому, що їх можна отримати навіть якщо аналітичний розв'язок неможливий. Майже всі нелінійні диференціальні рівняння в похідних похідних потрібно розв'язувати чисельними методами, і більшість реальних моделей фізики, хімії, біології тощо є нелінійними. Як правило, лінійні моделі наближаються до нелінійних лише якщо в них відкинути нелінійні члени. Наприклад, ось кілька нелінійних рівнянь:

1) нелінійне хвильове рівняння $ u_{tt} = u_{xx} + f(u) $ ;

2) рівняння реакції з нелінійною дифузією $ u_t = u_{xx} + f(u) $ ;

3) система Ходжкіна—Хакслі $ $$

u(x, t) =

$$ $ Аналітичні розв'язки для цих рівнянь не відомі для довільних нелінійних функцій f і $ g^{(1)} $ . Тому загальний підхід до розв'язання нелінійних (а в низці випадків і лінійних) задач зазвичай базується на чисельних розв'язках.

Тепер давайте розглянемо приклад того, як можна використати аналітичний розв'язок для визначення фізичних параметрів. Детальніший опис цих завдань можна знайти у рекомендованій літературі.

# Параметрична ідентифікація (у біології)
...
<sup>1)</sup> Це не так. Як приклад можна навести рівняння $ u_{tt} = u_{xx} - \sin u_x - \Pi u $ .

зв'язок між концентрацією калію $ u\left(x,\,t\right) $ і коефіцієнтом дифузії $ D $ , а потім за вимірюваннями величини $ u\left(x,\,t\right) $ визначити величину D.
...
Отже, параметри A і a слід обирати так, щоб крива відповідала експериментальним даним (наприклад, у сенсі методу найменших квадратів). Також було встановлено, що

![](images/_page_303_Figure_6.jpeg)

. 40.2. Початкова концентрація іонів ( $ ^{42}K $ ): a — теоретична крива; $ \delta $ — експериментальні точки.

з плином часу іони калію під дією дифузії та конвекції розтікаються вздовж аксона. Ходжкін і Кейс припустили, що концентрація u є розв'язком такої задачі з початковими даними для рівняння дифузії:
...
Звичайно, усе це дуже просто: не складніше, ніж скинути щось на стрічку транспортера, а потім спостерігати за його рухом. Однак усе стає значно цікавішим, якщо домішка дифундує в середовищі. Щоб розібратися, що відбувається з рухомою хвилею за наявності дифузії, розв'яжемо
...
$$ де, як звичайно, H(x) — функція Хевісайда. Початковий розподіл концентрації зображено на рис. 15.4.

![](images/_page_309_Figure_3.jpeg)

Рис. 41.1. Нова система координат  $ \xi = y - x $ ,  $ \eta = y $ .

КРОК 2. Залишилося знайти канонічну форму рівняння. Підставляючи  $ \xi $  і  $ \eta $  у формули для коефіцієнтів  $ \overline{A} $ ,  $ \overline{B} $ ,  $ \overline{C} $ ,  $ \overline{D} $ ,  $ \overline{E} $ ,  $ \overline{F} $  і  $ \overline{G} $ , отримуємо

$ \overline{A} = 0 $  (так і має бути; ми знаходили  $ \xi $  з умови рівності нулю цього коефіцієнта),

$ \overline{B}=0 $  (ми вже раніше показали, що цей коефіцієнт також дорівнює нулю),
...
Перед нами стоїть розв'язок проблеми конвективної дифузії (15.2). Це дуже легко інтерпретувати, якщо уявити, що ми рухаємося відносно графіка, показаного на рис. 15.6.
...
де константи $ \alpha $ і $ \beta $ визначаються з умов y(a) = A та y(b) = B (тобто ланцюгова лінія має проходити через задані точки). У цьому випадку досить складно розв'язати рівняння Ейлера-Лагранжа, тому ми рекомендуємо читачу лише перевірити, чи задовольняє його лінія ланцюга.
...
Основні ідеї калькулюсу варіацій можна розширити на функціонали, які залежать від функції кількох незалежних змінних вигляду П
...
Далі. Наближений розв'язок  $ u_n(x,y) = a_1 \varphi_1 + a_2 \varphi_2 + \dots + a_n \varphi_n $ .
На цьому етапі можна чисельно знайти значення функціонала  $ J[u_n] $ . Якщо необхідно знайти значення розв'язку  $ u_n(x,y) $  у деякій точці, можна написати невелику програму для виконання цієї операції. За потреби можна повторити обчислення за цією програмою з іншими значеннями n. Рис. 45.2. Блок-схема методу Рітца.
...
за допомогою сімейства задач, параметризованих числом $\varepsilon$, де $0 \le \varepsilon \le 1$.
...
<sup>4)</sup> Крайова задача

можна неперервно модифікувати в нелінійне рівняння $ \Delta u = u^2 = 0 $ 1).

Фіг. 46.1 показує схему розв'язання нелінійної задачі шляхом її зведення до розв'язку рівняння<sup>Лапласа 2</sup>)
