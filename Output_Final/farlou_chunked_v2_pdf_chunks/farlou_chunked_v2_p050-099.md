...
$$
\begin{aligned}
(\text{YHII})\quad & u_t = u_{xx} + \sin(\lambda_1 x), \quad 0 < x < 1,\quad 0 < t < \infty,\\
(\text{\GammaY})\quad & \begin{cases}
u\left(0, t\right) = 0,\\
u_x\left(1, t\right) + u\left(1, t\right) = 0,
\end{cases}\quad 0 < t < \infty,\\
(\text{HY})\quad & u\left(x, 0\right) = 0, \quad 0 \leqslant x \leqslant 1.
\end{aligned}
$$
методом розкладання за власними функціями, якщо $\lambda_1$ — перший корінь рівняння $\operatorname{tg}\lambda = -\lambda$. Якими є власні функції цієї задачі?
...
| 2. $f(x) = \begin{cases} 1, & -1 < x < 1, \\ 0, & x \le -1, x \ge 1 \end{cases}$ | $F(\xi) = \sqrt{\frac{2}{\pi}} \frac{\sin \xi}{\xi}$ (прямокутна функція). |
| 3. $f(x) = e^{-x^2}$ | $F\left(\xi\right)=\frac{1}{\sqrt{2}}e^{-\xi^2/4}$ (основна гаусова функція). |
