...
$$ або $$ u_t(\xi, t) = \frac{k}{c\rho} \left\{ \frac{u_x(x + \Delta x, t) - u_x(x, t)}{\Delta x} \right\} + \frac{1}{c\rho} f(\xi, t).
...
$$ (\Gamma Y) \quad \begin{cases} U(0, t) = 0, \\ U(L, t) = 0, \end{cases} \quad 0 < t < \infty,$$ (HY) \quad U(x, 0) = \varphi(x) - \left[k_{1} + \frac{x}{L}(k_{2} - k_{1})\right] = \overline{\varphi}(x),
...
$$ = \frac{\partial}{\partial t} \left[\frac{2}{\pi} \int_{0}^{\infty} u\left(x, t\right) \sin\left(\omega x\right) dx\right] =$$ = \frac{d}{dt} \mathcal{F}_{s}\left[u\right] =
...
$$\xi = y + ix^2, \quad \eta = y - ix^2.$$, $ \tau = t $ в формулы (15.3), получаем
...
$$u_p(r, \theta) = Ar^4 + Br^4 \cos(2\theta).$$ $ u(\xi, \eta) = 1/\eta + tg \xi, $ $ u(\xi, \eta) = \eta^2 + e $ являются решениями уравнения $ u_{\mathrm{ }nh}=0 $ . Второй шаг сделан. $ \text{IIIA}\Gamma $ 3 (Возвращение к старым координатам x и t).
...
$$(46.8) u = u_0 + \varepsilon u_1 + \varepsilon^2 u_2 + \dots.$$ $ u_1(x, 0) = 0. $ По формуле Даламбера получаем розв'язок
...
$$ F(\omega) = \frac{?}{\pi} \int_{0}^{\infty} f(x) \sin(\omega x) dx$$ u_{x}(0, t) = \frac{h}{T} u(0, t),
$$ 0 < \omega < x$$ u_{x}(L, t) = -\frac{h}{T} u(L, t).
$$ f''(0)$$ u_{x}(0, t) - \frac{h}{T}u(0, t) = 0,
$$ 2. $f(\alpha x)$ $$ u_{x}(L, t) + \frac{h}{T}u(L, t) = 0.
$$ 3. $e^{-\alpha x}$ $$
...
$$ -n^{2}C_{n} - \frac{2}{\pi} \{f'(0) - (-1)^{n} f'(n)\}$$ \begin{cases} u(x, 0) = x, \\ u_t(x, 0) = 0. \end{cases}
...
$$\Delta u = 0,$$ q_{uu} + q_{vv} = 0$$ \begin{cases} u(0, t) = 0, \\ u(L, t) = 0, \end{cases}

$$ w = f(z)$$ ![](_page_355_Figure_5.jpeg)
...
де $ a_n $ , $ b_n $ , $ R_n $ и $ \delta_n $ —произвольные постоянные. Заметим, що элементарные колебания удовлетворяют граничным условиям. Читателю должно быть ясно, що каждое элементарное колебание представляет собой стоячую волну. В стоячей волне все точки колеблются с одной и той же частотой, а форма хвилі подобна волнам, изображенным на рис. 20.3.
...
$$ = \ln\left[\frac{z-1}{z+1}\right] + i \arg\left[\frac{z-1}{z+1}\right]$$ = \arg\left[\frac{2y}{x^2+y^2-1}\right]$$

Воспользовавшись соотношениями ортогональности

$$\varphi(x, y) = \frac{1}{\pi} \arctan\left[\frac{2y}{x^2 + y^2 - 1}\right].$$ x^{2} + y^{2} = 1,$$ \begin{cases} 0, & m \neq n, \\ L/2, & m = n, \end{cases}
...
$$ # ТАБЛИЦЯ B. Перетворення Фур'є $i \sqrt{\frac{\pi}{2}} [\delta(\omega+a) - \delta(\omega-a)]$ $$ u(x, 0) = \sin(\pi x) + 0.5 \sin(3\pi x),
$$ F(\omega) = \frac{?}{\pi} \int_{0}^{\infty} f(x) \sin(\omega x) dx$$ a_n=0
$$ u_{x}(0, t) = \frac{h}{T} u(0, t),
$$ u(x, t) = \cos(\pi^2 t) \sin(\pi x) + 0.5 \cos(9\pi^2 t) \sin(3\pi x)
$$ 0 < \omega < x$$ u(x, t) = \cos(\pi t) \sin(\pi x) + 0.5 \cos(3\pi t) \sin(3\pi x)
$$ u_{x}(L, t) = -\frac{h}{T} u(L, t).
$$ u(0, t) = 0,
$$ f''(0)$$ u(x, t) = \sum_{n=1}^{\infty} X_n(x) \left[ a_n \sin(\omega_n t) + b_n \cos(\omega_n t) \right],
...
$$ 3. $e^{-\alpha x}$ $$ (\text{HV}) \quad

$$ [2/\pi\omega]^{1/2}$$ \begin{cases} u(x, 0) &= f(x), \\ u_t(x, 0) &= g(x). \end{cases}
...
$$e^{-1}a + \omega$$ $ u_t(x, 0) = \sin(\pi x). $ $ 0 \le x \le 1, $ 3. Решите задачу 2 с начальными условиями

$$\frac{1}{2}e^{-\alpha}\sin\omega$$ \frac{1-e^{-a\omega}}{\omega}$$ \begin{array}{l} u(x, 0) = 1 - x^2, \\ u_t(x, 0) = 0. \end{array}

$$ \quad 0 \le x \le 1,

$$ f(x) = \int_{0}^{\infty} F(\omega) \cos(\omega x) d\omega \qquad \qquad F(x) = \frac{2}{\pi} \int_{0}^{\infty} f(x) \cos(\omega x) dx$$ (см. рис. 22.2).
...
$$ Применяя це преобразование к предыдущей задаче (22.3), можна полностью перейти к безразмерным переменным U, $ \xi $ и $ \tau_i $ $$ Новая безразмерная система обладает следующими свойствами:
...
$$ -n^{2}C_{n} - \frac{2}{\pi} \{f'(0) - (-1)^{n} f'(n)\}$$ С помощью преобразования независимых змінних к безразмерному виду (преобразовывать u нет необходимости)
...
$$ | _                                |
$$ # ТАБЛИЦЯ B. Перетворення Фур'є $i \sqrt{\frac{\pi}{2}} [\delta(\omega+a) - \delta(\omega-a)]$ $$ \begin{cases} u(x, y, 0) = \varphi(x, y), \\ u_t(x, y, 0) = \psi(x, y), \end{cases}

$$ F(\omega) = \frac{?}{\pi} \int_{0}^{\infty} f(x) \sin(\omega x) dx$$

0 < \omega < x

$$ f''(0)$$ $ мы рассмотрим розв'язок трехмерной задачи при начальных условиях, зависящих лише от двух змінних x и y.
...
HIDE5d8af62e1c4f4f6ba05a4cac21331f96X п

![](_page_362_Figure_5.jpeg)

роследим, как оно решается методом конечного синус-преобразования. Вот что мы должны сделать: разложить еходное воздействие f(x, t) на компоненты, найти отклик  $ U_n $  на каждую компоненту, а затем сложить эти отклики. Математически это может быть и не совсем очевидно, но давайте внимательно проследим за ходом решения. Разложим входящие в уравнение с частными производными члены в ряды Фурье по синусам:
...
с. 46 3. Схематичне представлення методу збурення.
...
HIDE61f66b8dfeb14f258323795eb816c3abXu(r, \theta) = u_0(r, \theta) + \frac{1}{4}u_1(r, \theta)HIDEdf52f0f372c74e89b05a5270a00732d0X\Delta u = 0,HIDEe55b57df764f4f419191d134833a9103Xq_{xx} + \varphi_{uu} = 0HIDE6a9efb43ed374319ba6594334edbba00Xq_{uu} + q_{vv} = 0HIDE48b6c746cd6943dbbcb63d4cc09c34c9Xw = f(z)HIDE88649c83af8947d1a6fdaba036c40606X Р

![](_page_355_Figure_5.jpeg)

ис. 47.1. Площина комплексної змінної $ \boldsymbol{z} $ та некольорових формул бігунів H

![](_page_355_Figure_7.jpeg)

IDEee4bf36182d440f49d871289f18eb84fXu + iv = (x + iy)^2 = x^2 - y^2 + 2ixyHIDE664f1a8151e34dc8bce13b16ca410bfdX\left\{
...
HIDEf24ba026a4474925ae0c8528feb03dcbX Р

![](_page_357_Figure_11.jpeg)

ИС 47.3 Задача граничного значення у верхній половині площини.
...
HIDEfaeda56d9705461a95a4cd9337a9eb58X Проблема полягає в тому, щоб знайти конформне відображення, яке перетворює цю область на щось простіше, де нашу проблему легко розв'язати. У цьому випадку очевидно, що ця проста ділянка має бути кільцем. Відповідна експозиція H

![](_page_359_Figure_10.jpeg)

IDEe090a0a865ef45f8a116144a7817c525Xw = 2t \left[ \frac{z-s}{z-t} \right],HIDEd03970d04e86424c86966810053aff4cXu = \gamma [(x-s)(x-t)-\gamma^2],HIDEdcb4e084c8ad4da78d717e8d190ed312X
