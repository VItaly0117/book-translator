...
Наш наступний крок — вибрати функції $\xi=\xi(x,y)$ і $\eta=\eta(x,y)$ так, щоб коефіцієнти $\overline A$ та $\overline C$ у новому рівнянні обернулися на нуль. Це і дає канонічну форму гіперболічного рівняння. Отже, треба вимагати
...
Після ділення на $\xi_y^2$ та $\eta_y^2$ відповідно одержуємо характеристичні рівняння
...
Фіг. 23.2. Характеристичні координати для рівняння $u_{xx}-4u_{yy}+u_x=0$.
...
Нарешті, виражаючи $x$ і $y$ через $\xi$ та $\eta$, дістаємо канонічну форму

$$
u_{\xi\eta}=\frac{\eta u_{\xi}-\xi u_{\eta}}{2(\eta^2-\xi^2)}.
$$

Таким чином, задача зводиться до рівняння у характеристичних координатах, яке вже можна досліджувати стандартними методами.
# ЗАУВАЖЕННЯ
...
При бажанні можна виразити $\alpha$ і $\beta$ через початкові координати $x$ та $y$:
...
1. вона відокремлює хвильові, дифузійні та стаціонарні процеси;
2. саме для канонічних форм існує найбільше готових аналітичних методів;
3. чисельні схеми для гіперболічних задач зазвичай теж будують уже після такого перетворення.
...
Це відображення переводить верхню півплощину у смугу, для якої гранична задача має значно простіший вигляд. Далі розв'язок знаходять у новій області та повертають назад у змінні $x$ і $y$.
...
Фіг. 26.1. Основна ідея принципу суперпозиції: оператор $L$ діє лінійно, тому сумі входів відповідає сума виходів.
...
зручно розкладати на кілька простіших підзадач: окремо враховувати початкову умову, окремо неоднорідність $f(x,t)$ і окремо кожну з граничних умов.
...
МЕТА ЛЕКЦІЇ: Ввести поняття рівнянь першого порядку і пояснити, як метод характеристик перетворює задачу Коші для УЧП на систему звичайних диференціальних рівнянь.
...
На відміну від рівняння теплопровідності, де початковий вплив миттєво розмазується по всій області, для рівнянь першого порядку інформація переноситься вздовж певних кривих у площині $(x,t)$. Ці криві називають характеристиками.
...
Фіг. 27.2. Для хвильового рівняння початкове збурення в точці породжує дві хвилі, що поширюються вздовж двох характеристик.
...
Нехай $A$ — квадратна матриця порядку $n$ з різними власними значеннями
$\lambda_1,\lambda_2,\dots,\lambda_n$. Побудуємо матрицю
...
Після відокремлення змінних отримуємо задачу на власні значення для рівняння
Гельмгольца:
...
При $\mu\le 0$ відповідна крайова задача має лише тривіальний розв'язок, тому нас
цікавлять додатні власні значення.
...
має два лінійно незалежні розв'язки:
$J_n(\lambda r)$ — функцію Бесселя першого роду та $Y_n(\lambda r)$ — функцію Бесселя
другого роду. Тому загальний розв'язок має вигляд
...
Оскільки $Y_n(\lambda r)$ необмежена при $r=0$, у фізичній задачі про мембрану
залишається лише
...
Кожна така функція описує фундаментальну форму коливань круглої мембрани. Їй
відповідає частота
...
Отже, всі можливі коливання круглої мембрани можна розглядати як суперпозиції
власних форм $U_{nm}(r,\theta)$ з відповідними власними частотами.
...
Досить загальною задачею прикладної математики є визначення потенціалу в деякій
області простору, якщо всередині цієї області задано розподіл джерел $f(x,y)$. В
електростатиці потенціал у області $D$ визначається розподілом зарядів із щільністю
$f(x,y)$, а в теорії теплопровідності аналогічно задається джерелами тепла. Типова
двовимірна задача в крузі приводить до рівняння Пуассона.
...
Ми не випадково обрали нульові граничні умови. Якщо б потрібно було розв'язати задачу
в загальному випадку, з ненульовими граничними умовами та неоднорідним рівнянням,
то внесок граничних даних можна було б знайти за допомогою інтеграла Пуассона з
лекції 33.
...
де $q>0$ — стала. На межі потенціал підтримується рівним нулю, а лапласіан є сталим у
всій області. Рівняння Пуассона означає, що графік $u(r,\theta)$ має всюди однакову
опуклість. Геометрично його можна уявляти як тонку мембрану з нерухомим краєм, яку
прогинає зовнішня сила.

Тепер перейдемо до головної частини лекції: визначення функції Гріна і побудови
розв'язку рівняння (36.1). Але спершу введемо потенціали точкових джерел і стоків.

# Потенциалы точечных источников и стоков

Щоб розв'язати неоднорідне лінійне рівняння, достатньо зрозуміти відповідь системи на
одне точкове джерело. Тоді загальний розв'язок можна отримати підсумовуванням таких
елементарних внесків. У теплопровідності точкове джерело є місцем виділення тепла, а в
електростатиці — точковим зарядом.

Припустимо, що джерело інтенсивності $+q$ розміщене в початку координат. Тоді потік є
радіальним, а повний потік через коло радіуса $r$ дорівнює

$$
-\int_0^{2\pi} u_r(r)\,r\,d\theta = -2\pi r\,u_r(r).
$$

За законом збереження цей потік повинен дорівнювати кількості тепла, що генерується
всередині кола, тобто

$$
-2\pi r\,u_r(r)=q.
$$

Розв'язуючи це рівняння відносно функції $u(r)$, отримуємо двовимірний потенціал
точкового джерела.
...
## Новa система координат для параболічного прикладу
...
Початковий розподіл концентрації зображено на рис. 15.4. Цей результат легко інтерпретувати, якщо уявити, що ми рухаємося відносно графіка, показаного на рис. 15.6.
...
- Початкова точка — внутрішні точки сітки — □ — Кінцева точка $p_i$;
...
Ймовірність закінчення випадкового блукання в граничній точці $p_i$ і величина винагороди $g_i$

| Гранична точка $p_i$ | $P_A(p_i)$ — відносна частка блукань, що закінчились у точці $p_i$ | $g_i$ — величина винагороди при досягненні точки $p_i$ |
|---|---|---|
| 1 | 0,04 | 1 |
| 2 | 0,15 | 1 |
| 3 | 0,03 | 1 |
...
3. У цій лекції ми виявили, що функція
...
Зверніть увагу, що в перших двох задачах коефіцієнти є сталими. Читач повинен розуміти, що параметр $b$ має бути достатньо малим, а необережний ряд може відрізнятися від точного розв'язку.

![](images/_page_343_Figure_2.jpeg)

Рис. 45.2. Блок-схема методу Рітца.

Чи знаєте ви, як виглядає графік цієї функції на відрізку $[0, 1]$? Зверніть увагу, що синусоїдальне перетворення функції $f(x)$ визначається лише для додатних цілих чисел $n$.
...
Фіг. 46.1 показує схему розв'язання нелінійної задачі шляхом її зведення до розв'язку рівняння Лапласа.
...
Будь ласка, надсилайте свої коментарі щодо змісту книги, якості перекладу та оформлення за адресою видавництва «Мир».

# Фарлоу Стейпл

# ДИФЕРЕНЦІАЛЬНІ РІВНЯННЯ В ПОХІДНИХ ПОХІДНИХ ДЛЯ НАУКОВЦІВ І ІНЖЕНЕРІВ

Науковий редактор М. В. Хатунцева. Молодший науковий редактор Е. І. Качуліна. Художник В. Е. Карпов. Художній редактор В. І. Шаповалов. Технічний редактор Т. А. Максимова. Коректор Т. І. Стіфсева.
